...
*Количество файлов:* {len(list(config.SCAN_DIR.glob("*"))) if config.SCAN_DIR.exists() else 0}"""
    ...
    async def _handle_help_callback(self, query):
        ...
        try:
            # Используем HTML для избежания проблем с парсингом
            help_text = f"""🔧 <b>Помощь по использованию бота</b>

<b>Основные функции:</b>
• 🖨️ <b>Сканирование</b> - создает PDF/JPEG файл документа
• 🖨️ <b>Распечатать</b> - отправляет файлы на печать
• 📊 <b>Статус</b> - показывает состояние сканера и принтера
• 🗑️ <b>Очистка</b> - удаляет старые файлы сканирования

<b>Настройки сканирования:</b>
• Разрешение: {html.escape(str(config.SCAN_DPI))} DPI
• Режим: {html.escape(str(config.SCAN_MODE))}
• Формат: {html.escape(str(config.SCAN_FORMAT))}

<b>Печать файлов:</b>
<b>Способ 1 (через кнопку):</b>
• Нажмите кнопку "🖨️ Распечатать"
• Отправьте файл в ответ на сообщение бота

<b>Способ 2 (через упоминание):</b>
• Отправьте файл в группу
• Упомяните бота: @scan_2_telegram_bot
...
<b>Автоматика:</b>
• Файлы удаляются через {html.escape(str(config.CLEANUP_AFTER_HOURS))} часов
• Максимальный размер: {html.escape(str(config.MAX_FILE_SIZE_MB))} МБ
...
4. Используйте /status для проверки"""
            ...
            logger.info(f"Справка отправлена пользователю {query.from_user.id}")
        except Exception as e:
            logger.error(f"Ошибка отправки справки: {e}", exc_info=True)
            await query.answer("❌ Ошибка при отображении справки", show_alert=True)
